...
static int uclass_add(enum uclass_id id, struct uclass **ucp)
{
	...
	*ucp = NULL;
	...
	/* 根据id的值在 .u_boot_list_2_uclass_1 ~ .u_boot_list_2_uclass_3 段之间查找对应的对象 */
	...
	/* 实际调用的是 cALLOc(size_t n, size_t elem_size) */
	uc = calloc( 1, sizeof(*uc) );
	if (!uc){ return -ENOMEM; }

	/* 初始化时，uc_drv->priv_auto_alloc_size 没有被赋值 */
...
int uclass_get(enum uclass_id id, struct uclass **ucp)
{
	...
	*ucp = NULL;
	...
	if ( !uc ){ return uclass_add( id, ucp ); }
...
int uclass_find_device_by_seq(enum uclass_id id, 
                              int seq_or_req_seq,
			                  bool find_req_seq, 
							  struct udevice **devp)
{
	...
	*devp = NULL;
	...
	if (seq_or_req_seq == -1){ return -ENODEV; }
...
int uclass_resolve_seq(struct udevice *dev)
{
	...
	assert(dev->seq == -1);
	ret = uclass_find_device_by_seq(dev->uclass->uc_drv->id, dev->req_seq, false, &dup);
	if (!ret) {
	...
	for ( seq = 0; seq < DM_MAX_SEQ; seq++ ) {
		ret = uclass_find_device_by_seq(dev->uclass->uc_drv->id, seq, false, &dup);
		if (ret == -ENODEV)
			break;
		if (ret)
			return ret;
